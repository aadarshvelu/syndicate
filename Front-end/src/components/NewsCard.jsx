...
const SWIPE_DIST = 55        // px to trigger swipe
const SWIPE_VEL  = 0.35      // px/ms (~350px/s) to trigger swipe
...
export default function NewsCard({ card, cardIndex, isTop, stackOffset, onNext, onPrev }) {
  const x      = useMotionValue(0)
  const y      = useMotionValue(0)
  const rotate = useTransform(x, [-260, 260], [-7, 7])
  // Fade as card moves left (goes transparent before flying off) or right (resists)
  ...
  const dismissed = useRef(false)  // guard against double onNext()
  ...
  useEffect(() => {
    if (!isTop) return
    setPlayState(flying || expanded ? 'paused' : 'running')
  }, [flying, expanded, isTop])

  // Guarded next — progress bar and swipe both call this; only first wins
  const safeNext = () => {
  ...
  const bind = useDrag(
    ({ movement: [mx, my], velocity: [vx], last, active }) => {
      if (!last) {
        // Live drag: set motion values directly
        // Clamp right movement to 28px max so back card never shows
      ...
      const swipeUp     = my < -60 && Math.abs(my) > Math.abs(mx)
    ...
    {
      from: () => [x.get(), y.get()],  // continue from current card position
      filterTaps: true,
      enabled: isTop && !flying,
    }
  )
  ...
  return (
    <>
      ...
      >
        ...
        {/* Content panel — top anchored so it always overlaps the hero */}
        <div style={{
      ...
      <AnimatePresence>
        {expanded && <FullArticleSheet card={card} onClose={() => setExpanded(false)} />}
      </AnimatePresence>
    </>
  )
}
